...
test("reflows vertical group sections at the boundary before drop", async ({
  ...
  await expect(designSection).toHaveCSS("border-radius", "12px");
  await expect(designSection).toHaveCSS("outline-offset", "3px");
...
test("enters grouped selection from the clicked element and switches only from the header control", async ({
  ...
  await expect(searchSection).toHaveCSS("border-radius", "12px");
  await expect(searchSection).toHaveCSS("outline-offset", "4px");
  await expect
