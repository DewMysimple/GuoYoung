...
export function GroupSortDragPreview({
  ...
  return (
    ...
    >
      ...
      {axis === "vertical" && (sites.length ? <div className="group-sort-preview-sites">
        {sites.slice(0, 3).map(site => <div key={site.id} className="group-sort-preview-site">
          <Favicon site={site} /><span>{site.name}</span>
        </div>)}
        {sites.length > 3 && <span className="group-sort-preview-more">+{sites.length - 3}</span>}
      </div> : <div className="group-sort-preview-empty">空分组</div>)}
    </div>
  );
}
